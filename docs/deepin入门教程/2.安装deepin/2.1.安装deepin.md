...
* 访问deepin社区[下载页面](https://www.deepin.org/download/)，下载深度操作系统系统最新版本的镜像文件（以便您能够体验到最新特性）
  ...
  <img src="image-20230410152303309.png" alt="image-20230410152303309" style="zoom: 50%;" />

可能你一看到这么多镜像，不知道选哪个。不要怕，如果你不知道RISC-V是什么，首先排除它。然后v23目前还没稳定发布，也排除它。

::: warning
deepin v23 目前处于测试阶段，不建议您在生产环境中使用，也不建议新手用户使用。
:::

那么你选v20.8下载就对了，下面任意一种下载方式都行。
...
md5校验是为了确保你的安装镜像下载完整了

* 下载深度操作系统镜像完成后，需要对其进行校验，非官方或不完整的镜像将不能用于深度操作系统的安装：

* Windows系统：下载下面几个工具中的任意一个，校验您下载的镜像的MD5值与[下载页面](https://www.deepin.org/download/)提供的 MD5 值是否一致。( MD5 值在**立即下载**按钮下方，也就是上图中蓝色的MD5SUMS，点开就能看到了)
...
* Linux系统：在对应的镜像文件下，打开深度终端，执行`md5sum deepin-20.8-amd64.iso`命令，请确认下载的镜像的 MD5 值与[下载页面](https://www.deepin.org/download/)提供的 MD5 值是否一致。( MD5 值在**立即下载**按钮下方)
    ...
    > 说明：deepin-20.8-amd64.iso即为下载的系统镜像文件名，可使用 Tab 键自动补全文件名。

## 制作启动盘

**下载制作工具**

* [深度启动盘制作工具](https://www.deepin.org/original/deepin-boot-maker/)（推荐）
...
* 开源多镜像引导工具
 ...
 4. tip：自deepin v20以后的系统，不再支持传统引导方式。
